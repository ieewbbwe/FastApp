...
import com.android_mobile.core.ui.listener.INavigationBar;
...
public class NavigationBar implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (callback == null)
            return;
        callback.navigationBarClickHandler(v.getId());
        callback.navigationBarClickHandler(v);
    }
    ...
    public void setLeftIcon(Drawable left, Drawable top, Drawable right,
        ...
        if (left != null) {
            left.setBounds(0, 0, act.dip2px(w), act.dip2px(h));
            leftBtn.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
        }
        if (right != null) {
            right.setBounds(0, 0, act.dip2px(w), act.dip2px(h));
            leftBtn.setGravity(Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        }
        if (top != null)
            top.setBounds(0, 0, act.dip2px(w), act.dip2px(h));
        if (bottom != null)
            bottom.setBounds(0, 0, act.dip2px(w), act.dip2px(h));
        leftBtn.setCompoundDrawables(left, top, right, bottom);
    }
    ...
    public float getHeight() {
        return act.getResources().getDimension(R.dimen.navigationbar_height);
    }
